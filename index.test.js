...
describe('calculateEnergyUsageSimple', () => {
  ...
  it('should calculate correctly when the appliance is initially on but switched off immediately', () => {
    const usageProfile5 = {
      initial: 'on',
      events: [{ timestamp: 0, state: 'off' }],
    };
    expect(calculateEnergyUsageSimple(usageProfile5)).toEqual(0);
  });

  it('should calculate correctly when the appliance is initially on but switched off at the end of the day', () => {
    const usageProfile8 = {
      initial: 'on',
      events: [{ timestamp: 1440, state: 'off' }],
    };
    expect(calculateEnergyUsageSimple(usageProfile8)).toEqual(1440);
  });

  it('should calculate correctly for out of order events with initial state = "off"', () => {
    const usageProfile6 = {
      initial: 'off',
      events: [
        { timestamp: 30, state: 'on' },
  ...
  it('should calculate correctly for out of order events with initial state = "on"', () => {
    const usageProfile7 = {
      initial: 'on',
      events: [
        { timestamp: 833, state: 'on' },
    ...
    expect(calculateEnergyUsageSimple(usageProfile7)).toEqual(
      126 + (1440 - 833)
    );
  });
